...
OPERATOR_METADATA = {
    ...
    OP_NOT_EQUAL: ('!=', "Binary, LHS attribute of the cell and RHS value are not equal"),
    OP_IN: ('<<', "Binary, LHS attribute of the cell equals one of the comma-separated values on RHS"),
    OP_NOT_IN: ('!<', "Binary, LHS attribute of the cell is not equal to any of the comma-separated values on RHS"),
...
class NeuronDB(object):
    def __init__(self, data_file_rows):
        ...
        self.search_index = SearchIndex([(nd['tag'], searchable_labels(nd), k) for k, nd in self.neuron_data.items()])
    ...
    def search(self, search_query, case_sensitive=False, word_match=False):
        ...
        chaining_rule, free_form_terms, structured_terms = NeuronDB._parse_search_query(search_query)
        log(f"Processing search query: '{chaining_rule=}' '{free_form_terms=}' '{structured_terms=}'")
        term_search_results = [
            self.search_index.search(term=term, case_sensitive=case_sensitive, word_match=word_match)
        ...
        if structured_terms:
            ...
            term_search_results.append([k for k, v in self.neuron_data.items() if predicate(v)])

        return NeuronDB._apply_chaining_rule(chaining_rule=chaining_rule, term_search_results=term_search_results)

    @lru_cache
    def search_in_neurons_with_inherited_labels(self, search_term, case_sensitive=False, word_match=False):
        if search_term:
            matching_doc_ids_set = set(self.search(
                search_query=search_term, case_sensitive=case_sensitive, word_match=word_match))
            return [p for p in self.rids_of_neurons_with_inherited_tags if p in matching_doc_ids_set]
        else:
    ...
    def closest_token_from_inherited_tags(self, term, case_sensitive):
        return self.search_index.closest_token(term, case_sensitive=case_sensitive,
    ...
    def _raise_unsupported_attr_for_structured_search(attr_name):
        raise ValueError(f"Structured query by attribute <b>{attr_name}</b> is not supported. Possible solutions:"
    ...
    def _make_predicate(structured_term):
        if structured_term['op'] == OP_EQUAL:
            return NeuronDB._make_equality_predicate(lhs=structured_term['lhs'], rhs=structured_term['rhs'])
        elif structured_term['op'] == OP_NOT_EQUAL:
            eqp = NeuronDB._make_equality_predicate(lhs=structured_term['lhs'], rhs=structured_term['rhs'])
            ...
            predicates = [NeuronDB._make_equality_predicate(lhs=structured_term['lhs'], rhs=i) for i in rhs_items]
    ...
    def _make_structured_terms_predicate(chaining_rule, structured_terms):
        predicates = [NeuronDB._make_predicate(structured_term=t) for t in structured_terms]
    ...
    def _extract_search_operators(term, ops=None):
        if not (term.startswith('\"') and term.endswith('\"')):
            ops = ops or SEARCH_TERM_BINARY_OPERATORS + SEARCH_TERM_UNARY_OPERATORS + SEARCH_CHAINING_OPERATORS
            ops_shorthands = [OPERATOR_METADATA[op][0] for op in ops]
            return [op for i, op in enumerate(ops) if (op in term or ops_shorthands[i] in term)]
        else:
            return []
    ...
    def _parse_chained_search_query(search_query):
        if search_query and not (search_query.startswith('\"') and search_query.endswith('\"')):
            search_operators = NeuronDB._extract_search_operators(search_query, SEARCH_CHAINING_OPERATORS)
            if len(search_operators) == 1:
                parts = NeuronDB._extract_operands(text=search_query, op=search_operators[0])
