...
def asset(filename):
    return send_from_directory(os.path.join(base.root_path, '../../static/assets'), filename)


@base.route('/styles/<path:filename>')
def styles(filename):
    return send_from_directory(os.path.join(base.root_path, '../../static/styles'), filename)
...
def index(path):
    if path:
        ...
        return redirect(url_for('app.search', filter_string=request.args.get('filter_string')))
    else:
